...
class CustomOrderFragment : androidx.fragment.app.Fragment(), AdapterView.OnItemSelectedListener {
    ...
    private fun isValidBeverage(): Boolean {
        return orderViewModel.retrieveEspressoShotCounter() > 0
    }

    private fun showOrderErrors() {
        if (orderViewModel.retrieveEspressoShotCounter() < 1) Toast.makeText(activity?.applicationContext, "A minimum of 1 Espresso shot is required for each order", Toast.LENGTH_LONG)
                .apply {
                    show()
                }
    }
    ...
    inner class Handlers {
        ...
        val detractEspressoShotListener: View.OnClickListener
            get() {
                return View.OnClickListener {
                    if (orderViewModel.retrieveEspressoShotCounter() > 0) {
